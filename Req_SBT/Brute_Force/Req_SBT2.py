...
if __name__ == '__main__':
    ...
    for round_idx in range (total_round):
        ...
        if round_idx == 0:
            goal_index = 0
            Configuration = CarBehindAndInFrontConfigure(goal_index,population,search_round)
        ...
        print_function_values_to_file(front, os.path.join(target_dir, '/FUN.' + str(round_idx) + '_' + algorithm.label))
        print_variables_to_file(front, os.path.join(target_dir, '/VAR.'+ str(round_idx) + '_' + algorithm.label))
