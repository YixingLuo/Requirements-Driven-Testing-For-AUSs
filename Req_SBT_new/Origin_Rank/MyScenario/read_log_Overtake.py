...
def evaluate_traffic_light (ego_vehicle_state, traffic_light):
    ...
    if len(ego_vehicle_state) <= index:
        ...
        if ego_vehicle_state[index][1] > t_start + 2: ## not start y >= 200.8
            satisfaction = 1
    elif len(ego_vehicle_state) > index_2:
        if ego_vehicle_state[index_2][1] <= t_start - 3 : ## t_start = 200 ego vehicle reach the destination before 196 y<=196
